   ...
   fetch(`https: //ev.io/stats-by-un/${encodeURIComponent(username)}`)
      ...
      .then(response => response.json()).then(data => {
         ...
         if (data.length === 0) {
            ...
            const crosshairUrl = `$ {
						user.field_custom_crosshair[0].url
					}
            ...
            userInfoDiv.innerHTML = ` <h3>User Information for $ {
						user.name[0].value
					}

					</h3> <p><strong>Username:</strong> $ {
						user.name[0].value
					}

					</p> <p><strong><i class="fa-solid fa-link" style="color: #ffffff;" ></i> User URL: </strong><a style="text-decoration: none; color: #FFA500;" href="${uidUrl}" target="_blank" >https: //ev.io/user/${user.uid[0].value}</a></p>

					<p><strong><i class="fa-solid fa-skull" style="color: #ffffff;" ></i> Kills:</strong> $ {
						user.field_kills[0].value.toLocaleString()
					}

					| <strong><i class="fa-solid fa-skull-crossbones" style="color: #ffffff;" ></i> Deaths:</strong> $ {
						user.field_deaths[0].value.toLocaleString()
					}

					| <strong><i class="fa-solid fa-person-rifle" style="color: #ffffff;" ></i> KD:</strong> $ {
						user.field_k_d[0].value
					}

					</p> <p><strong><i class="fa-solid fa-ranking-star" style="color: #ffffff;" ></i> Ranking:</strong> $ {
						user.field_rank[0].value.toLocaleString()
					}

					| <strong><i class="fa-solid fa-star" style="color: #ffffff;" ></i> Score:</strong> $ {
						user.field_score[0].value.toLocaleString()
					}

					</p> <p><strong><img src="src/e_coin.png" alt="Sol Logo" style="width: 15; height: 15px; margin-bottom: 6px;" > Coins:</strong> $ {
					...
					<img src="src/usdc-coin.svg" alt="Sol Logo" style="width: 20; height: 20px; margin-bottom: 6px;" ></p><hr> <p><strong><i class="fa-solid fa-crosshairs" style="color: #ffffff;" ></i> Crosshair:</strong> <a style="text-decoration: none; color: #FFA500;" href="${crosshairUrl}" target="_blank" >Users Crosshair</a></p> `;
            ...
            if (skinId) {
               fetch(`https: //ev.io/node/${skinId}?_format=json`)
                  ...
                  .then(response => response.json()).then(skinData => {
                     const skinImage = skinData.field_wallet_image[0].url;
                     const skinInfo = document.createElement('div');
                     skinInfo.innerHTML = `<br> <img src="${skinImage}" alt="${user.name[0].value}'s Skin" style="max-width: 200px; border-radius: 25px;" > `;
...
const showMoreInfo = () => {
   ...
   if (user) {
      ...
      const userapi = `https: //ev.io/stats-by-un/${user.name[0].value}`;
      const userapiuid = `https: //ev.io/user/${user.name[0].value}?_format=json`;

      moreInfoDetails.innerHTML = ` <p><strong>Abilities Layout:</strong> $ {
					user.field_abilities_loadout[0].value
				}

				</p> <p><strong>Total Games:</strong> $ {
					user.field_total_games[0].value
				}

				</p> <p><strong>Wallet Address:</strong> <span style="text-decoration: underline;" >$ {
					user.field_wallet_address[0].value
				}

				<span></p> <p><strong>Weekly Score:</strong> $ {
					user.field_weekly_score[0].value
				}

				| <strong>CP Earned Weekly:</strong> $ {
					user.field_cp_earned_weekly[0].value
				}

				</p> <p><strong>CP Lifetime Earned:</strong> $ {
					user.field_lifetime_cp_earned[0].value
				}
...
searchButton.addEventListener('click', () => {
   ...
   fetch(api_url).then(response => {
      ...
      infoContainer.innerHTML += `<br><br> <p><strong>Skin Name:</strong> $ {
							skin_name
						}

						</p> <p><strong><i class="fa-solid fa-link" style="color: #ffffff;" ></i> Game Node URL:</strong> <a style="text-decoration: none;" href="${uidUrl}" target="_blank" >$ {
							uidUrl
						}

						</a></p> <p><strong><i class="fa-solid fa-fire" style="color: #ffffff;" ></i> Weapon Type:</strong> $ {
							weapon_type
						}

						</p> <p><strong><i class="fa-solid fa-hourglass-half" style="color: #ffffff;" ></i> Reset Time:</strong> $ {
							reset_time
						}

						</p> <p><strong><i class="fa-solid fa-square" style="color: #ffffff;" ></i> Collection:</strong> $ {
							collection
						}
...
window.addEventListener('load', async () => {
   ...
   try {
      ...
      const earnRateDataHtml = ` <center> <h2><img src="src/ev_icon.png" alt="Sol Logo" style="width: 30; height: 30px; margin-bottom: 6px;" >Earn Rate</h2> <p>The E Earn Rate Per 100 Score is: <strong>$ {
							field_e_earn_rate_per_100_score
						}

						</strong></p> <p>The Earnings Cap is: <strong>$ {
							field_earnings_cap
						}
...
window.addEventListener('load', async () => {
   ...
   try {
      ...
      const nftDropsList = nftDropData.map(nft => ` <center> <div class="nft-drop" style="text-transform: capitalize;" > <h2>$ {
								nft.type
							}
								...
								nft.field_drop_chance
							}

							</p> <p>Quantity Left: $ {
								nft.field_quantity_left
							}
...
function calculateUSDCFromE() {
   ...
   if (!isNaN(eCoinAmount) && eCoinAmount > 0) {
      ...
      document.getElementById('usd-result').textContent = `USDC Value: $$ {
						usdValue.toFixed(2)
					}
...
document.addEventListener("DOMContentLoaded", () => {
   ...
   const displaySkin = (tabId, skinName, skinIcon) => {
      ...
      tab.innerHTML = ` <h2>Equipped $ {
															tabId.replace('Tab', '')
														}

														Skin</h2> <p><strong>Skin Name:</strong> $ {
															skinName
														}

														</p> <img src="${skinIcon}" alt="${skinName} Skin" style="max-width: 300px;" > <hr class="modern-hr" > `;
...
function fetchCryptocurrencyInfo() {
   fetch("https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd%2Cinr%2Cbdt%2Cphp%2Cbrl%2Cmyr%2Caud&include_market_cap=false&include_24hr_vol=false&include_24hr_change=true&include_last_updated_at=true&precision=2").then(response => response.json()).then(data => {
      const usd = data.solana.usd;
      const inr = data.solana.inr;
      const php = data.solana.php;

      document.getElementById('usd').textContent = `1 SOL=$ {
															usd
														}

														USD`;

      document.getElementById('inr').textContent = `1 SOL=$ {
															inr
														}

														INR`;

      document.getElementById('php').textContent = `1 SOL=$ {
															php
														}
...
window.addEventListener('load', function() {
  ...
  }, 3000); // Hide loading screen after 3 seconds if the user doesn't click the skip button
});
